...
def process_data(ds, nbin, fs):
    ...
    Suu = fft_tool.calc_psd(ds["vel"][0], freq_units="Hz").sel(freq=slc_freq)
    Svv = fft_tool.calc_psd(ds["vel"][1], freq_units="Hz").sel(freq=slc_freq)
    Szz = (Suu + Svv) / (2 * np.pi * Suu["freq"]) ** 2  # deep water approx

    uvz = xr.DataArray(
        np.array((ds["vel"][0].data, ds["vel"][1].data, ds["z"].data)),
        dims=["dirUVZ", "time"],
        coords={"dirUVZ": ["u", "v", "z"], "time": ds.time},
    )
    ...
    a = Cxz.values / np.sqrt((Suu + Svv) * Szz)
    b = Cyz.values / np.sqrt((Suu + Svv) * Szz)
    theta = np.arctan2(b, a)  # degrees CCW from East, "to" convention
    phi = np.sqrt(2 * (1 - np.sqrt(a**2 + b**2)))
    phi = np.nan_to_num(phi)  # fill missing data

    direction = np.arange(Szz.size)
    spread = np.arange(Szz.size)
    for i in range(0, Szz.size):
        # degrees CW from North
        direction[i] = (270 - np.rad2deg(np.trapz(theta[i], Szz.freq))) % 360
        # degrees
